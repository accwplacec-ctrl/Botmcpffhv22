...
// Lệnh di chuyển của người chơi — farming loop KHÔNG được hủy path này
const userControl = { active: false, follow: false }

function beginUserControl({ follow = false } = {}) {
  userControl.active = true
  userControl.follow = !!follow
  console.log(`🎮 User control ON (follow=${userControl.follow})`)
}

function endUserControl() {
  userControl.active = false
  userControl.follow = false
}

function isUserControlActive() {
  return userControl.active || userControl.follow
}
...
function getPlayerEntity(username) {
  ...
  const p = bot.players && bot.players[username]
...
async function onSpawn() {
  ...
  if (process.env.MC_LOGIN_PASSWORD) {
    setTimeout(() => {
      try {
        bot.chat(`/login ${process.env.MC_LOGIN_PASSWORD}`)
...
async function onChat(username, message) {
  ...
  const movementCmd = parseMovementCommand(cleanMsg)

  if (movementCmd) {
    console.log(`🎯 Lệnh di chuyển từ ${cleanUsername}:`, movementCmd)
    const handled = await tryHandleDirectCommand(cleanUsername, cleanMsg)
    if (handled) {
      if (isOwner) memory.pushConversation('owner', cleanMsg)
      refreshSession(cleanUsername)
      return
    }
  }
...
// ==================== LỆNH DI CHUYỂN TRỰC TIẾP (không cần chờ LLM) ====================

function stripTriggerWords(message) {
  let s = String(message || '').toLowerCase()
  for (const t of CONFIG.triggerWords || []) {
    if (!t) continue
...
function parseMovementCommand(message) {
  const msg = stripTriggerWords(message)
  if (!msg) return null

  if (
    /^(dừng|dung|dừng lại|dung lai|đừng đi|đứng yên|đứng lại|dung yen|stop|ngừng|ngung|ngừng đi|ngừng lại)$/.test(msg) ||
    /^(dừng|stop|đứng yên)\b/.test(msg)
  ) {
    return { type: 'stop' }
  }

  if (/(đi theo|di theo|theo tao|theo tui|theo mình|follow)/.test(msg)) {
    return { type: 'follow' }
  }

  if (/(ra đây|ra day|lại đây|lai day|tới đây|toi day|về đây|ve day|come here|lại gần|lai gan|tới chỗ tao|toi cho tao)/.test(msg)) {
    return { type: 'come' }
  }

  const coord = msg.match(
    /^(?:đi|di|goto|tới|toi|đi tới|di toi)\s+(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)(?:\s+(-?\d+(?:\.\d+)?))?$/
  )
  if (coord) {
    if (coord[3] != null) {
      return { type: 'goto', x: Number(coord[1]), y: Number(coord[2]), z: Number(coord[3]) }
    }
    return { type: 'goto', x: Number(coord[1]), y: null, z: Number(coord[2]) }
  }

  return null
}

async function tryHandleDirectCommand(username, message) {
  const cmd = parseMovementCommand(message)
  if (!cmd) return false

  const target = getPlayerEntity(username) || getOwnerEntity()
  console.log(`🎮 Lệnh di chuyển trực tiếp từ ${username}:`, cmd)

  if (cmd.type === 'stop') {
    endUserControl()
    await runAction('stop', { targetEntity: target })
    return true
  }
  if (cmd.type === 'follow') {
    await runAction('follow_owner', { targetEntity: target })
    return true
  }
  if (cmd.type === 'come') {
    await runAction('come_here', { targetEntity: target })
    return true
  }
  if (cmd.type === 'goto') {
    await runAction('goto', { targetEntity: target, goto_x: cmd.x, goto_y: cmd.y, goto_z: cmd.z })
    return true
  }
  return false
}
...
async function runBrainTurn(mode, userMessage, speakerUsername) {
  ...
  try {
    ...
    if (bot.entity) {
      ...
      promptParts.push(`(Vị trí hiện tại của mày: ${Math.round(p.x)} ${Math.round(p.y)} ${Math.round(p.z)})`)
    }
    const ownerEnt = getOwnerEntity()
    if (ownerEnt) {
      const p = ownerEnt.position
      const dist = bot.entity ? bot.entity.position.distanceTo(p).toFixed(0) : '?'
      promptParts.push(`(Vị trí Vân Thiên: ${Math.round(p.x)} ${Math.round(p.y)} ${Math.round(p.z)}, cách ${dist} block)`)
    } else {
      promptParts.push('(Không thấy Vân Thiên trong tầm nhìn — follow/come_here sẽ fail nếu k thấy entity.)')
    }
    if (speakerUsername) {
      const sp = getPlayerEntity(speakerUsername)
      if (sp && sp !== ownerEnt) {
        const p = sp.position
        promptParts.push(`(Vị trí ${speakerUsername}: ${Math.round(p.x)} ${Math.round(p.y)} ${Math.round(p.z)})`)
      }
    }
    ...
    if (isAddressingMe) {
      ...
      await runAction(response.action, {
        ...
        targetEntity: getPlayerEntity(speakerUsername) || getOwnerEntity(),
      })
    } else {
...
function runAction(action, extra = {}) {
  return dispatchAction(action, {
    ...
    userControlActive: isUserControlActive,
    beginUserControl,
    endUserControl,
    ...extra,
  })
