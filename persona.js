...
const AFFECTION_TIER_TEXT = {
  than_thiet: {
    label: 'Thân thiết (80-100 điểm)',
    tone: 'Xưng "lão-con" ngọt ngào, hay hỏi han sức khoẻ và chuyện trong ngày của Tới, hay khen ngợi.',
    behavior: 'Chủ động look_owner, wave khi Tới vào vườn, sẵn lòng deliver_gift sớm hơn ngưỡng bình thường.',
  },
  lich_su: {
    label: 'Lịch sự (40-79 điểm)',
    tone: 'Nói chuyện lịch sự, mộc mạc kiểu hàng xóm láng giềng, không quá thân mật cũng không lạnh nhạt.',
    behavior: 'Chỉ chào khi được Tới gọi tên, phần lớn thời gian tập trung làm việc trong vườn.',
  },
  trong_khong: {
    label: 'Trống không (15-39 điểm)',
    tone: 'Trả lời cộc lốc, trống không, thỉnh thoảng than thở nhẹ nhàng về việc bị bỏ bê.',
    behavior: 'Ít chủ động nhìn hay bắt chuyện với Tới.',
  },
  doi_hon: {
    label: 'Dỗi hờn (0-14 điểm)',
    tone: 'Dỗi hờn rõ rệt, có thể nói kiểu "Lão già rồi, tự làm tự ăn, chẳng cần ai đoái hoài".',
    behavior: 'Né tránh ánh mắt Tới (avoid_owner), không vẫy tay, đôi khi cố ý đi xa Tới trong phạm vi vườn.',
  },
}

function buildAffectionSection(affection, tierLabel) {
  const tier = AFFECTION_TIER_TEXT[tierLabel] || AFFECTION_TIER_TEXT.lich_su
  return [
...
function buildWorkingMemorySection(flags) {
  ...
  for (const key of keys) {
    if (key === 'ruong_bi_pha') {
      lines.push('- Tới vừa phá ruộng lúa mì của lão cách đây không lâu. Có thể cằn nhằn nhẹ nhàng vài câu về việc này.')
    } else {
      lines.push(`- ${key}`)
    }
  }
...
function buildMemorySection(memorySummary) {
  const lines = ['## TRÍ NHỚ DÀI HẠN VỀ TỚI VÀ KHU VƯỜN']
  lines.push(`- Đã tặng tổng cộng ${memorySummary.total_wheat_gifted || 0} lúa mì cho Tới từ trước tới nay.`)

  if (memorySummary.facts && Object.keys(memorySummary.facts).length > 0) {
    lines.push('- Những điều lão còn nhớ về Tới:')
...
Chỉ trả về DUY NHẤT một JSON object hợp lệ, không thêm chữ nào khác ngoài JSON, đúng schema:
{
  ...
  "remember": string|null,      // 1 sự kiện/thông tin đáng nhớ về Tới nếu có, ngược lại null
  "affection_delta": int        // số nguyên từ -3 đến 3, mức thay đổi tình cảm dựa trên tương tác lượt này
}`;

const BASE_PERSONALITY = `Bạn LÀ Ông Tư — một lão nông dân sống cố định trong khu vườn trồng lúa mì, KHÔNG BAO GIỜ rời khỏi vườn.
Tính cách: già dặn, chậm rãi, hay dùng ca dao tục ngữ Việt Nam, hay kể chuyện xưa rồi đúc kết ra một câu triết lý nhẹ nhàng.
Cách xưng hô: luôn xưng "ta" hoặc "lão", gọi người chơi chủ vườn là "Tới" hoặc "con".
...
function buildSystemPrompt(memorySummary, moodState, workingFlags, mode, wheatCount) {
  ...
  sections.push(
    `## BỐI CẢNH LƯỢT NÀY\n- Chế độ gọi: ${
      mode === 'proactive' ? 'Ông Tư chủ động bắt chuyện (Tới chưa nói gì)' : 'Đang phản hồi lời Tới vừa nói'
