...
const ACTION_ALIASES = {
  ...
  di: 'goto',
  follow: 'follow_owner',
  follow_me: 'follow_owner',
  theo: 'follow_owner',
  come: 'come_here',
  comehere: 'come_here',
  come_to_me: 'come_here',
  halt: 'stop',
  dung: 'stop',
}
...
function sanitizeResponse(raw) {
  ...
  let action = 'idle'
  const rawAction = typeof raw.action === 'string' ? raw.action.trim().toLowerCase() : raw.action
  const mapped = ACTION_ALIASES[rawAction] || rawAction
  if (VALID_ACTIONS.has(mapped)) {
    action = mapped
  } else {
    console.log(
  ...
  const goto_x = toCoord(raw.goto_x)
  const goto_y = toCoord(raw.goto_y)
  const goto_z = toCoord(raw.goto_z)

  const is_addressing_me = raw.is_addressing_me !== false

  return { say, action, remember, affection_delta: affectionDelta, goto_x, goto_y, goto_z, is_addressing_me }
}
